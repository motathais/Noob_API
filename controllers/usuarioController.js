...
const usuarioController = {
  create: async (req, res) => {
    try {
      ...
      // Função para normalizar e validar o apelido
      const normalizeApelido = (apelido) => {
        const limpo = apelido.replace(/[^a-zA-Z0-9]/g, "");
        return `@${limpo.toLowerCase()}`;
      };

      const isApelidoValido = (apelido) => /^@[a-z0-9]+$/.test(apelido);

      const apelidoNormalizado = normalizeApelido(apelido);

      if (!isApelidoValido(apelidoNormalizado)) {
        return res.status(400).json({
          message:
            "Formato de apelido inválido. Use apenas letras e números, começando com @",
        });
      }

      // Verificação de existência de email e apelido (case-insensitive)
      ...
      if (usuarioExiste) {
        return res.status(401).json({
          message: "O email inserido está em uso, por gentileza utilize outro.",
        });
      }
      ...
      if (apelidoExiste) {
        return res.status(401).json({
          message:
            "O apelido inserido já está em uso, por gentileza utilize outro.",
        });
      }
  ...
  delete: async (req, res) => {
    try {
      ...
      const deletedUsuario = await Usuarios.findByIdAndDelete(id).select(
        "-senha"
      );
  ...
  update: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ updatedUsuario, msg: "Usuário atualizado com sucesso!" });
    } catch (error) {
      console.log(error);
      res
        .status(500)
        .json({ msg: "Erro ao processar a atualização do usuário." });
    }
  },
};
